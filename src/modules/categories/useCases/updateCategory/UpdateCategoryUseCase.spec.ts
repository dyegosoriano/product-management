...
describe('UpdateCategoryUseCase', () => {
  ...
  it('the category must contain a minimum of 3 letters and a maximum of 30', async () => {
    await expect(updateCategoryUseCase.execute(category_id, { name: '01' })).rejects.toThrow(ZodError)

    await expect(
      updateCategoryUseCase.execute(category_id, { name: '0123456789 ABCDEFGHIJKLMNOPQRSTUVZWYZ' })
    ).rejects.toThrow(ZodError)
  })

  it('should return an error if an invalid uuid is entered', async () => {
    await expect(updateCategoryUseCase.execute('invalid_uuid', { name: 'category_name' })).rejects.toThrow(ZodError)
  })

  it('should return an error if the category was not found', async () => {
    await expect(updateCategoryUseCase.execute(category_id, { name: 'category_name' })).rejects.toEqual(
  ...
  it('should be possible to update a category', async () => {
    const category = await updateCategoryUseCase.execute(category_id, { name: 'new_category_name' })
